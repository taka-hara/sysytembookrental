...
class isbnSearch
{
    ...
    public function search($isbn)
    {
        ...
        if($this->isbnCheck()){
            ...
            if(!is_null($result_rss->channel->item[0])) {
                ...
                $this->result['error_message'] = '検索結果がありません。';
                return $this->result;
            }
        }
        return $this->result;
    ...
    private function isbnCheck()
    {
        //ISBNが空でないか
        if($this->isbn === '') {
            $this->result['error_message'] = 'ISBNを入力してください。';
            return false;
        }
        //ISBNが数値か
        if(!preg_match("/^[0-9]+$/",$this->isbn)) {
            $this->result['error_message'] = 'ISBNは半角数字で入力してください。';
            return false;
        }
        //入力されたISBNの長さを取得
        ...
        if((($isbn_length != 13) && ($isbn_length != 10))) {
            $this->result['error_message'] = 'ISBNは10桁もしくは13桁で入力してください。';
            return false;
        }
        return true;
    }
